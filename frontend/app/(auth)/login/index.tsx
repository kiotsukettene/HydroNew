...
export default function Login() {
  ...
  //     const userInfo = await GoogleSignin.signIn();
  //     const { idToken } = userInfo; // ✅ Correct property

  //     if (!idToken) {
  //       throw new Error('No ID token returned from Google');
  //     }

  //     // 3️⃣ Create Firebase credential using Google ID token
  //     const googleCredential = auth.GoogleAuthProvider.credential(idToken);

  //     // 4️⃣ Sign in to Firebase
  //     const userCredential = await auth().signInWithCredential(googleCredential);

  //     // 5️⃣ Get Firebase ID token (verifiable by Laravel backend)
  //     const firebaseIdToken = await userCredential.user.getIdToken(true);

  //     console.log('✅ Firebase ID Token:', firebaseIdToken.slice(0, 30) + '...');

  //     // 6️⃣ Send Firebase token to backend
  //     await googleLogin(firebaseIdToken);
  //   } catch (error: any) {
  //     if (error.code === statusCodes.SIGN_IN_CANCELLED) {
  //       console.log('🚫 User cancelled sign-in');
  //     } else if (error.code === statusCodes.IN_PROGRESS) {
  //       console.log('⚙️ Sign-in in progress');
  //     } else if (error.code === statusCodes.PLAY_SERVICES_NOT_AVAILABLE) {
  //       console.log('❌ Play services not available');
  //     } else {
  //       console.error('Google login error:', error);
